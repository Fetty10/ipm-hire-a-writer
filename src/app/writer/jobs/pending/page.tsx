...
export default function WriterPendingJobs() {
  ...
  async function handleStart(id:string) {
    ...
    const data = await res.json();
    if(res.ok){ setJobs(prev=>prev.filter(j=>j.id!==id)); router.push("/writer/jobs/active"); }
  ...
  const nav = WRITER_NAV.map(item=>item.href==="/writer/jobs/pending"?{...item,badge:jobs.length}:item);
