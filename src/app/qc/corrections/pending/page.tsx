          ...
          return (
            <div key={job.id} style={C.card}>
              ...
              <div style={C.files}>
                ...
                {job.guidelineFileUrl && job.guidelineFileUrl.split(",").map((u:string,i:number,arr:string[]) => (<a key={i} href={`/api/download/guideline?url=${encodeURIComponent(u.trim())}&label=${encodeURIComponent(`Client File${arr.length>1?` ${i+1}`:""} ${job.topic}`)}`} target="_blank" rel="noreferrer" style={C.flink}>⬇ Client File{arr.length>1?` ${i+1}`:""}</a>))}
                {!job.submittedFileUrl && !supUrl && !job.guidelineFileUrl && <p style={{fontSize:".78rem",color:"#5B7EA6",fontStyle:"italic"}}>No files uploaded yet.</p>}
              </div>
