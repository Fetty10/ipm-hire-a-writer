...
{job.guidelineFileUrl && typeof job.guidelineFileUrl === "string" && job.guidelineFileUrl.split(",").filter(Boolean).map((u:string,i:number,arr:string[]) => (<a key={i} href={`/api/download/guideline?url=${encodeURIComponent(u.trim())}&label=file`} target="_blank" rel="noreferrer" style={C.flink}>⬇ Client File{arr.length>1?` ${i+1}`:""}</a>))}
