...
const C = {
  ...
  grid2:  { display:"grid", gridTemplateColumns:"3fr 2fr", gap:"1rem" },
...
export default function WriterDashboard() {
  ...
  const nav = WRITER_NAV.map(item=>{
    if(item.href==="/analyst/jobs/pending") return {...item,badge:pending.length};
    if(item.href==="/analyst/jobs/active")  return {...item,badge:active.length};
    return item;
  });
  ...
  return (
    <StaffLayout navItems={nav} role="Analyst" initials={initials}>
      <div style={C.page}>
        <h1 style={C.h1}>Analyst Dashboard</h1>
          ...
          <>
            <div style={C.grid4}>
              {[
                ...
                {icon:"⏳",val:pending.length,   label:"Pending",      href:"/analyst/jobs/pending"},
                {icon:"✍️",val:active.length,    label:"Active",       href:"/analyst/jobs/active"},
                {icon:"✅",val:delivered.length, label:"Delivered",    href:"/analyst/jobs/delivered"},
              ].map(s=>(
            ...
            <div style={C.grid2}>
              ...
              <div style={C.card}>
                <div style={C.chead}>
                  ...
                  <button style={C.clink} onClick={()=>router.push("/analyst/jobs/active")}>View all →</button>
                </div>
                {active.length===0
