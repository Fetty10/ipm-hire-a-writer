...
export async function POST(req: NextRequest) {
  ...
  if (event.event === "charge.success") {
    ...
    // Parse metadata — Paystack sends it as string or object
    ...
    if (meta?.isAddChapters && meta?.addChapters?.length) {
      // Save the additional payment amount
      await prisma.order.update({
        where: { id: orderId },
        data: {
          amountPaidKobo: { increment: amount },
          status:         "IN_PROGRESS",
        },
      });

      // Assign only the new chapters
