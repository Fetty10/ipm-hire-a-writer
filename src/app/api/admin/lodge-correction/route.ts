...
// Admin lodges a manual correction job for a legacy client.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let student = await prisma.user.findFirst({
      where: { email: studentEmail.trim(), role: Role.CLIENT },
    });
    ...
    console.error("[LODGE CORRECTION ERROR]", err?.message, err?.stack);
    return NextResponse.json(
