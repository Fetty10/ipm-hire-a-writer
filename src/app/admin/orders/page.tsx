...
const C = {
  ...
  card:   { background:"#fff", borderRadius:"16px", border:"1.5px solid #E0F2FE", overflow:"hidden", marginBottom:".6rem" },
  row:    { display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto auto", gap:"1rem", alignItems:"center", padding:".85rem 1.25rem", cursor:"pointer", borderBottom:"1px solid #F0F9FF" },
  ...
  chevron:{ color:"#5B7EA6", fontSize:"1rem" },
  // Detail panel
  panel:  { position:"fixed" as const, top:0, right:0, width:"520px", height:"100vh", background:"#fff", borderLeft:"1.5px solid #E0F2FE", boxShadow:"-8px 0 32px rgba(0,0,0,.1)", zIndex:40, overflowY:"auto" as const, padding:"1.5rem" },
  ...
  close:  { background:"none", border:"none", fontSize:"1.25rem", cursor:"pointer", color:"#5B7EA6" },
  ...
  irow:   { display:"flex", justifyContent:"space-between", fontSize:".8rem", marginBottom:".3rem" },
  ilbl:   { color:"#5B7EA6" },
  ival:   { fontWeight:600, color:"#0C1A2E" },
...
function OrderDetail({ orderId, onClose, staffList }: { orderId:string, onClose:()=>void, staffList:any[] }) {
  ...
  async function act(action:string, extra:any={}) {
    ...
    const res  = await fetch("/api/admin/orders",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,orderId,...extra})});
    const data = await res.json();
        ...
        <div style={C.sect}>
          ...
          <div style={C.infoBox}>
            {[
              ...
              <div key={r.label} style={C.irow}><span style={C.ilbl}>{r.label}</span><span style={C.ival}>{r.val}</span></div>
            ))}
          </div>
        </div>
        ...
        <div style={C.sect}>
          ...
          <div style={C.infoBox}>
            {[
              ...
              <div key={r.label as string} style={C.irow}><span style={C.ilbl}>{r.label}</span><span style={C.ival}>{r.val as any}</span></div>
            ))}
          </div>
        </div>
        ...
        <div style={C.sect}>
          ...
          {order.chapters?.map((ch:any)=>(
            <div key={ch.id} style={C.chCard}>
              <div style={C.chHead}>
                <div style={{...C.chNum,...(ch.status==="DELIVERED"?{background:"#38BDF8",color:"#0C1A2E"}:{})}}>{ch.chapterNumber}</div>
                <div style={C.chInfo}>
              ...
              <div style={{display:"flex",gap:"1rem",marginBottom:".5rem"}}>
                {ch.submittedFileUrl&&<a href={ch.submittedFileUrl} target="_blank" rel="noreferrer" style={C.dlBtn}>📄 Submitted</a>}
                {ch.deliveredFileUrl&&<a href={ch.deliveredFileUrl} target="_blank" rel="noreferrer" style={C.dlBtn}>✅ Delivered</a>}
              </div>
...
function AdminOrdersContent() {
  ...
  useEffect(()=>{
    fetch("/api/admin/staff?filter=active").then(r=>r.json()).then(d=>{ if(d.success) setStaffList(d.data); });
  },[]);
    ...
    <AdminLayout>
      <div style={C.page}>
        ...
        {loading ? <div style={{textAlign:"center",padding:"3rem",color:"#5B7EA6"}}>Loading...</div> : (
          orders.map((o:any)=>{
            const writerNames = [...new Set(o.chapters?.map((ch:any)=>ch.assignedTo?.name).filter(Boolean))] as string[];
              ...
              <div key={o.id} style={C.card} onClick={()=>setSelected(o.id)}>
                <div style={C.row}>
                  ...
                  <div>
                    ...
                    <div style={C.rmeta}>{delivered}/{o.chapters?.length||0} chapters delivered</div>
                  </div>
